// forked from design-sync lib/source-kit.mjs - treat Next.js dynamic route segments ([id]) as generic dirs so they never become component groups
    ...
    for (const c of components) {
      ...
      c.group = slug(
        slash(relative(srcRoot, dirname(hit)))
          .split('/')
          .filter((s) => s && s.toLowerCase() !== c.name.toLowerCase() && !GENERIC_DIR.has(s.toLowerCase()) && !/^\[.*\]$/.test(s))
          .at(-1)
