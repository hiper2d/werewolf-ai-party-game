...
- **App owns policy** (`app/ai/ai-models.ts` overlay): free-tier bands and per-game caps, `DEPRECATED_MODEL_MAP`/`resolveModelId`, `RANDOM`, image models, story-generation settings, the voice metadata lists (`app/ai/voice-config/`: which voices exist per provider, gender, descriptions for casting); `app/ai/agent-factory.ts` wraps the library factory and wires `setLlmLogger` to BetterStack
