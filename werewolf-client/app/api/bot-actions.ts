...
import {botReminderPostfix, botSystemPrompt, BOT_VOTE_PROMPT, BOT_AFTER_GAME_SYSTEM_PROMPT_ADDITION, replyLengthInstruction} from "@/app/ai/prompts/bot-prompts";
...
function generateBotSystemPrompt(bot: Bot, game: Game): string {
    const basePrompt = format(botSystemPrompt(game.gameMode), {
        name: bot.name,
...
async function handleHumanPlayerMessage(
    ...
    // Persist the human's message FIRST, before the GM selection call. Previously
    ...
    // Ask the GM which bots should respond. selectRespondingBots reads the saved
    // day messages (including the message just persisted above), clamps invalid
    // names, and enforces BOT_SELECTION_CONFIG.MIN — so we never set an empty queue.
    const selectedBots = await selectRespondingBots(game, apiKeys, userEmail);
    ...
    try {
        ...
        // Mode 2: VOTE state - Process voting
            ...
            const botPrompt = format(
                botSystemPrompt(currentGame.gameMode),
                {
                    name: bot.name,
                    personal_story: bot.story,
                    play_style: "",
            ...
            const playStyleReminder = format(botReminderPostfix(currentGame.gameMode), { play_style: generatePlayStyleDescription(bot), human_player_name: currentGame.humanPlayerName, reply_length_instruction: replyLengthInstruction(currentGame.longReplies) });
            const history = convertToAIMessages(bot.name, [...botMessages, gmMessage]);
            history.push({ role: MESSAGE_ROLE.USER, content: playStyleReminder.trim() });
