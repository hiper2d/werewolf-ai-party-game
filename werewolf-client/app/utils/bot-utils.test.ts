...
describe('getEffectiveModel', () => {
    ...
    it('applies a pending override for the matching player', () => {
        const game = gameWith({ botName: 'Jace', model: 'claude-fable-5', enableThinking: true });
        expect(getEffectiveModel(game, 'Jace', 'mistral-medium-3', false))
            .toEqual({ aiType: 'claude-fable-5', enableThinking: true });
    });

    it('ignores an override targeting a different player', () => {
        const game = gameWith({ botName: 'Selkie', model: 'claude-fable-5' });
    ...
    it('uses the override thinking flag, not the base flag, when overriding', () => {
        const game = gameWith({ botName: 'Jace', model: 'claude-fable-5' });
        expect(getEffectiveModel(game, 'Jace', 'mistral-medium-3', true))
            .toEqual({ aiType: 'claude-fable-5', enableThinking: false });
    });

    it('supports the Game Master as an override target', () => {
        const game = gameWith({ botName: GAME_MASTER, model: 'claude-fable-5' });
