        ...
        <>
            <div className="space-y-2">
                {Object.entries(apiKeys).length === 0 ? (
                    <p className="text-sm text-[var(--fg-1)] italic">No API keys configured yet.</p>
                ) : (
                    Object.entries(apiKeys).map(([model, apiKey]) => (
                        <div key={model} className="flex items-center justify-between py-2 border-b border-[var(--line-1)]">
                            <div className="flex items-center gap-3 min-w-0">
                                <span className="font-medium text-sm">{getFriendlyName(model)}</span>
                                <span className="text-xs text-[var(--fg-1)] font-mono">{maskKey(apiKey)}</span>
                            </div>
                            <div className="flex items-center gap-1 flex-shrink-0">
                                <button
                                    onClick={() => { setEditingKey(model); setEditValue(apiKey); }}
                                    className="p-1.5 rounded hover:hover:bg-[var(--bg-3)] transition-colors"
                                ...
                                >
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-red-600 text-[var(--danger)]">
            ...
            {(editingKey || isAdding) && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div
                        ref={modalRef}
                        className="relative bg-[rgb(var(--color-card-bg))] border-[var(--line-1)] border rounded-lg p-6 w-full max-w-md mx-4 shadow-xl"
                    >
                        ...
                        <div className="flex justify-end gap-2">
                            <button
                                onClick={closeModal}
                                className="px-4 py-2 text-sm rounded border-[var(--line-1)] border hover:hover:bg-[var(--bg-3)] transition-colors"
                            >
                                Cancel
                            </button>
                            <button
                                ...
                                className="px-4 py-2 text-sm rounded bg-[var(--accent)] hover:brightness-110 text-[var(--on-accent)] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                            >
                                {isAdding ? 'Add' : 'Save'}
                            </button>
                        </div>
