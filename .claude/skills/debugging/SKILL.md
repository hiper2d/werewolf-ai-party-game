...
- `rebuild-bot-prompt.ts <gameId> <botName>` — rebuilds the bot's CURRENT discussion-reply prompt exactly as `processNextBotInQueue` does (system prompt with both cache tiers + the `convertToAIMessages` history + the trailing playstyle reminder) and writes it to `logs/<bot>-day<N>-prompt.txt`. Use it to answer "what does this bot actually see".
- `request-stats-report.ts [--days N]` — per-model cache-hit ratio, latency percentiles and $/request from the `requestStats` collection (one doc per AI call, with `cachedInputTokens`). Caches are per MODEL and per provider TTL; measured 2026-09-06 that human-paced games (12-78 min between a bot's calls) got 0-16% hits on Claude/OpenAI and 20-43% on DeepSeek/GLM/Qwen/MiniMax, whose caches live longer.
